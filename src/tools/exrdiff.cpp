...
int main(int argc, char *argv[]) 
{
    ...
    for (int i = 0; i < 4*r1[0]*r1[1]; ++i) {
        if (diffImage) 
            diffImage[i] = ((im1[i] - im2[i]) * (im1[i] - im2[i]))/(im2[i]*im2[i]+0.01f);
        if (im1[i] == 0 && im2[i] == 0) 
            continue;
        ...
        mse += ((im1[i] - im2[i]) * (im1[i] - im2[i]))/(im2[i]*im2[i]+0.01f);
        if (d > .005) ++smallDiff;
        if (d > .05) ++bigDiff;
    }
...
static void WriteEXR(const char *name, float *pixels, int xRes, int yRes) {
    ...
    for (int i = 0; i < xRes * yRes; ++i) {
        ...
        if(sum < 0.25f) {
            ...
            newR = 0.5f+std::max(sum-1.f, 0.f);
            newG = 2.f*std::max(1.f-sum, 0.f);
        }
        //hrgba[i] = Rgba(pixels[4*i], pixels[4*i+1], pixels[4*i+2], 1.);        
        hrgba[i] = Rgba(newR, newG, newB, 1.);
    }
